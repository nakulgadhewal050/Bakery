...
exports.getUsers = async (req, res) => {
  ...
  const orderCountMap = new Map(
    orderCounts.map((item) => [String(item._id), item.totalOrders])
  );

  const usersWithOrderCount = users.map((user) => ({
    ...user,
    totalOrders: orderCountMap.get(String(user._id)) || 0,
  }));
...
exports.downloadOrderInvoice = async (req, res) => {
  try {
    ...
    if (!order.user) {
      return res.status(400).json({ success: false, message: "User data not found for this order" });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="Invoice_${order._id.toString().substring(0, 12)}_${Date.now()}.pdf"`
    );
