...
const OrderSuccess = () => {
  ...
  const handleDownloadInvoice = async () => {
    try {
      ...
      const orderId = orderDetails.dbOrderId || orderDetails.orderId;
  ...
  useEffect(() => {
    const initializeOrder = async () => {
      try {
        ...
        console.log("🔄 Initializing order:", { sessionId, dbOrderId, paymentId });

        const token = localStorage.getItem("token");
        let orderId = dbOrderId || `ORD${Date.now()}`;
        let orderItems = [];
        let orderTotal = parseFloat(amount) || 0;

        // Step 1: Verify Stripe payment on backend
        if (sessionId && dbOrderId) {
          try {
            const verifyRes = await fetch(`/api/orders/verify-payment`, {
              method: "POST",
              headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
              body: JSON.stringify({ sessionId, orderId: dbOrderId }),
            });
            const verifyData = await verifyRes.json();
            if (verifyData.success) {
              console.log("✅ Payment verified:", verifyData.order._id);
            }
          } catch (verifyErr) {
            console.warn("⚠️ Verify call failed:", verifyErr.message);
          }
        }
          ...
          try {
            ...
            if (orderRes.ok) {
              ...
              if (orderData.success) {
                console.log("✅ Order loaded from DB:", orderData.order._id);
                orderId = orderData.order._id;
                orderItems = orderData.order.items || [];
                orderTotal = orderData.order.totalAmount || orderTotal;
              }
            }
          } catch (dbError) {
            console.warn("⚠️ DB fetch failed:", dbError.message);
          }
        }

        // Step 3: Fallback to Redux cart items
        if (orderItems.length === 0 && items && items.length > 0) {
          orderItems = [...items];
        }

        // Step 4: Clear pending order from localStorage
        ...
        setOrderDetails({
          ...
          total,
          timestamp: new Date().toISOString(),
          paymentStatus: (sessionId || paymentId) ? "Paid" : "Pending",
          orderStatus: "Confirmed",
          paymentId: sessionId || paymentId,
        });

        console.log("✅ Order details set");
        localStorage.setItem("lastOrder", JSON.stringify({ orderId, dbOrderId, items: orderItems, total, timestamp: new Date().toISOString() }));
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:sticky lg:top-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <button
                  ...
                  disabled={
                    downloadingInvoice || orderDetails.paymentStatus !== "Paid"
                  }
                  className={`w-full py-2 sm:py-3 rounded-lg sm:rounded-xl font-semibold transition-colors flex items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base cursor-pointer ${
                    downloadingInvoice || orderDetails.paymentStatus !== "Paid"
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl sm:rounded-2xl p-4 sm:p-6">
              ...
              <div className="space-y-1 sm:space-y-2">
                <button
                  onClick={() => window.open("tel:+919111525695", "_blank")}
                ...
                <button
                  onClick={() =>
                    window.open("mailto:nakulgadhewal0@gmail.com", "_blank")
                  }
                  className="w-full py-1.5 sm:py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors text-xs sm:text-sm"
                >
                  ✉️ Email Support
