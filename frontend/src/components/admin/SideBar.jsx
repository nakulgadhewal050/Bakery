...
const SideBar = ({ closeSidebar }) => {
  ...
  return (
    <>
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed top-0 left-0 h-full w-56 sm:w-64 bg-[#fff0f3] shadow-xl z-50 transform transition-transform duration-300 flex flex-col ${mobileOpen || closeSidebar
            ? "translate-x-0"
        ...
        <nav className="flex-1 mt-3 px-2 space-y-1">
          {menus.map((m) => (
            <Link
              ...
              to={m.path}
              onClick={handleClose}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition ${isActive(m.path)
